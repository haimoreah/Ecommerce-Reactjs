...
| `reel-skeptic` | محامي الشيطان — يحاول إسقاط الريل | — نقض فقط |
...
7. **`reel-skeptic` إلزامي قبل أي APPROVE.** يُشغَّل **بعد** اللجنة ويرى أحكامها ليهاجمها
   (هذا الاستثناء الوحيد من قاعدة الاستقلال — دوره النقض لا التقييم).
   لم يصمد الريل ⇒ **لا APPROVE**. صمد ⇒ ارفع الثقة، لا الدرجة.

### تسجيل الدرجة — إلزامي

عند نشر أي ريل، سجّل في `data/reels.csv`: `predicted_score` و`predicted_verdict`.
**هذه هي الحلقة التي تحوّل النظام من رأي إلى متنبئ مُقاس.** بدونها لا تعرف أبدًا
إن كانت الـrubric تتنبأ بشيء. `analyze.mjs` يحسب ارتباط الرتب بين الدرجة والأداء الفعلي،
وسيخبرك صراحةً إذا كانت البوابة **مقلوبة** أو الدرجات **متضاغطة**.

إذا قال السكربت إن الدرجة غير متنبئة ⇒ **قل ذلك للمستخدم في كل تقرير**،
وعامل الدرجة كرأي منظّم لا كدليل.
